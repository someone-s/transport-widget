...
class ArrivalsRdgApi(
    ...
    override fun fetchLatestArrivals(context: Context): List<ArrivalModel> {
        ...
        if (response == null) {
            ...
            return emptyList()
        } else if (response.body() !is RdgDepartureResponse) {
            Log.i(this.javaClass.name, "no body")
            return emptyList()
        }

        val services = (response.body() as RdgDepartureResponse).services

        val responseTimeFormatter = DateTimeFormatter.ofPattern("uuuu-MM-dd'T'HH:mm:ss")

        return services
            .filter { it.hasActualTime || it.hasEstimatedTime || it.hasScheduledTime }
            .map { service ->
